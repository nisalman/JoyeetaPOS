...
class TransferController extends Controller
{
    ...
    public function set(Request $request)
    {
        ...
        $sale_list_by_shops = DB::table('shop_sales')
        ...
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        return view('disbursement.set', compact('warehouse_id', 'start_date',
            'end_date', 'lims_purchase_data', 'lims_product_purchase_data', 'lims_sale_data',
            ...
            'lims_expense_data', 'sale_list_by_shops'));
    }

    public function makeBatch(Request $request)
    {
        $transID = $request->transID;
        $batchId = $request->storeID . Carbon::now()->format('dmyhms');
        ...
        return redirect()->action('TransferController@batchList');

    }

    public function batchList()
    {
        $transData = DB::table('shop_sales')
            ->where('is_disburse', '0')
            ->where('batch_id', '!=', '0')
            ->get();
        ...
        foreach ($groupTrans as $keys => $froupTran) {
            ...
            foreach ($froupTran as $key => $tran) {
                ...
                if ($keys == $tran->batch_id) {
                    $totalPay = $totalPay + $tran->paid_amount;
                }
            }

            $warehouseName = Warehouse::find($tran->wearhouse_id)->name;
            ...
            $trans[$keys]['ekshop_commission'] = 10;
            $trans[$keys]['final_pay'] = 10;
            $trans[$keys]['store_id'] = $tran->wearhouse_id;
            $trans[$keys]['store_name'] = $warehouseName;
            $trans[$keys]['transactionID'] = $tran->batch_id;
        ...
        return view('disbursement.batch_list', compact('trans'))->with('successMsg', 'Batch Successfully created');
    }
    ...
    public function batchPayment(Request $request)
    {
        ...
        return view('disbursement.final_disbursement', compact('request'));
    ...
    public function confirmDisburse(Request $request)
    {
        ...
        $disburseId = $request->storeID . Carbon::now()->format('dmyhms');


        DB::table('disburse_lists')
            ->insert([
                'batch_id' => $request->batchID,
                'disburse_id' => $disburseId,
                'warehouse_id' => $request->storeID,
    ...
    public function disburseHistory()
    {
        $disbursements = DB::table('disburse_lists')
            ->get();
        return view('disbursement.disburse_list', compact('disbursements'));
